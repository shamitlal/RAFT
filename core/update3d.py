...
class BasicUpdateBlock3D(nn.Module):
    def __init__(self, args, hidden_dim=128, input_dim=128):
        super(BasicUpdateBlock3D, self).__init__()
        self.args = args
        ...
        self.invdep_head = nn.Sequential(
            nn.Conv2d(128, 256, 3, padding=1),
            nn.ReLU(inplace=True),
    ...
    def forward(self, net, inp, corr, flow, residual_depth, upsample=True):
        # flow = flow.permute(0,3,1,2)
        # residual_depth = residual_depth.permute(0,3,1,2)

        motion_features = torch.cat([flow, 10*residual_depth], dim=1)
        ...
        net = self.gru(net, inp, corr_features, motion_features)

        revisions_flow = self.revisionsnet(net)
        revision_invdep = self.invdep_head(net)

        mask = .25 * self.mask(net)
        mask_invdep = .25 * self.mask_invdep(net)

        
        return net,  mask, mask_invdep, revisions_flow, revision_invdep,
